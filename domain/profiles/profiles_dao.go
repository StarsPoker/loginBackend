...
const (
	...
	queryGetProfileValidation     = "SELECT id, name, profile_code, withdrawal, expense, bot, closure, atendence, finish_withdrawal, create_manual_order FROM profiles WHERE profile_code = ?"
	queryGetProfile               = "SELECT id, name, profile_code, withdrawal, expense, bot, closure, atendence, finish_withdrawal, create_manual_order FROM profiles WHERE id = ?"
	...
	queryUpdateParam              = "UPDATE profiles SET withdrawal = ?, expense = ?, bot = ?, closure = ?, atendence = ?, finish_withdrawal = ?, create_manual_order = ? WHERE id = ?"
	...
	queryGetProfileUsersAdds      = "SELECT id, name, status, (select id_profile from profile_users where id_user = u.id) FROM users u where u.id not in(select id_user from profile_users where id_profile = ?)"
	...
	queryGetProfileAttendants     = "SELECT id, name, status, (select id_profile from profile_users where id_user = u.id) FROM users u where u.id not in(select id_user FROM profile_users where id_profile = ?)"
	...
	queryGetProfilePermissions    = "SELECT id, name, profile_code, withdrawal, expense, bot, closure, atendence, finish_withdrawal, create_manual_order FROM profiles WHERE profile_code = ?"
	...
	queryGetProfileByUser         = "SELECT p.id, p.name, p.profile_code, p.withdrawal, p.expense, p.bot, p.closure, p.atendence, p.finish_withdrawal, create_manual_order FROM profiles p JOIN profile_users pu ON p.id = pu.id_profile where pu.id_user = ?"
)
...
func (p *Profile) GetProfileValidation() *rest_errors.RestErr {

	stmt, err := stars_mysql.Client.Prepare(queryGetProfileValidation)
	if err != nil {
		logger.Error("error when trying to prepare profile validation statement", err)
		return rest_errors.NewInternalServerError("database error")
	}
	defer stmt.Close()

	result := stmt.QueryRow(p.ProfileCode)
	if getErr := result.Scan(&p.Id, &p.Name, &p.ProfileCode, &p.Withdrawal, &p.Expense, &p.Bot, &p.Closure, &p.Atendence,
		&p.FinishWithdrawal, &p.CreateManualOrder); getErr != nil {
		logger.Error("error when trying to get profile validation", getErr)
		return rest_errors.NewInternalServerError("database error")
	}
...
func (p *Profile) GetProfile() *rest_errors.RestErr {
	...
	if getErr := result.Scan(&p.Id, &p.Name, &p.ProfileCode, &p.Withdrawal, &p.Expense, &p.Bot, &p.Closure, &p.Atendence, &p.FinishWithdrawal, &p.CreateManualOrder); getErr != nil {
...
func (p *Profile) UpdateParam() *rest_errors.RestErr {
	...
	_, updateErr := stmt.Exec(&p.Withdrawal, &p.Expense, &p.Bot, &p.Closure, &p.Atendence, &p.FinishWithdrawal, &p.CreateManualOrder, &p.Id)
...
func (p *Profile) GetProfilePermissions() *rest_errors.RestErr {
	stmt, err := stars_mysql.Client.Prepare(queryGetProfilePermissions)

	if err != nil {
		logger.Error("error when trying to prepare get profile permission statement (GetProfilePermissions)", err)
		return rest_errors.NewInternalServerError("database error")
	}
	defer stmt.Close()
