...
const (
	...
	queryGetProfileByUser         = "SELECT p.id, p.name, p.profile_code, p.withdrawal, p.expense, p.bot, p.closure, p.atendence, p.finish_withdrawal, p.make_blocked_withdrawal, p.make_alert_withdrawal FROM profiles p JOIN profile_users pu ON p.id = pu.id_profile where pu.id_user = ?"
)
