...
const (
	...
	queryGetProfile               = "SELECT id, name, profile_code, withdrawal, expense, bot, closure, atendence, finish_withdrawal, make_blocked_withdrawal, make_alert_withdrawal, finish_atendance FROM profiles WHERE id = ?"
	...
	queryUpdateParam              = "UPDATE profiles SET withdrawal = ?, expense = ?, bot = ?, closure = ?, atendence = ?, finish_withdrawal = ?, make_blocked_withdrawal = ?, make_alert_withdrawal = ?, finish_atendance = ? WHERE id = ?"
	...
	queryGetProfilePermissions    = "SELECT id, name, profile_code, withdrawal, expense, bot, closure, atendence, finish_withdrawal, make_blocked_withdrawal, make_alert_withdrawal, finish_atendance FROM profiles WHERE profile_code = ?"
...
func (p *Profile) GetProfile() *rest_errors.RestErr {
	...
	if getErr := result.Scan(&p.Id, &p.Name, &p.ProfileCode, &p.Withdrawal, &p.Expense, &p.Bot, &p.Closure, &p.Atendence, &p.FinishWithdrawal, &p.MakeBlockedWithdrawal, &p.MakeAlertWithdrawal, &p.FinishAtendance); getErr != nil {
...
func (p *Profile) UpdateParam() *rest_errors.RestErr {
	...
	_, updateErr := stmt.Exec(&p.Withdrawal, &p.Expense, &p.Bot, &p.Closure, &p.Atendence, &p.FinishWithdrawal, &p.MakeBlockedWithdrawal, &p.MakeAlertWithdrawal, &p.FinishAtendance, &p.Id)
